...
describe('description rendering', () => {
  it.each(['pasta with tomato', 'pasta with meat'])
    ('renders description from property value "%s"', (descriptionText) => {
...
describe('available portions rendering', () => {
  it.each([1, 36])
    ...
    (availablePortions) => {
      ...
      expect(portionCounter.text().split('/')[1]).toBe(availablePortions.toString());
    })
});
...
describe('portion price rendering', () => {
  it.each([8, 15])
    ...
    (portionPrice) => {
      ...
      expect(portionPriceElement.text().startsWith(portionPrice.toString())).toBe(true);
    })
});
